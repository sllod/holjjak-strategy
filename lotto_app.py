...
-  **연속번호**: 3개 이상 연속 시 제거  
  ↳ 실제 로또 1등 번호 통계에서 3개 이상 연속 번호가 나올 확률은 약 2% 미만으로 매우 낮습니다. 따라서 연속 번호가 많은 조합은 제거하여 현실적인 조합만 추천합니다.

-  **홀/짝 비율**: 선택한 비율만 허용 (AI 추천 포함)  
  ↳ 참고: 1등 당첨 번호의 약 80% 이상이 홀3:짝3 또는 홀4:짝2 비율입니다. 극단적인 홀짝 조합(예: 홀6:짝0, 홀5:짝1)은 역사적으로 거의 나오지 않았습니다.

-  **번호 구간 분포(숫자 분포)**: 1~45 범위에서 고르게 분포하도록 필터링  
  ↳ 당첨 번호는 특정 구간(예: 1~10)에 몰리지 않고, 여러 구간에 분산되는 경향이 강합니다. 최소 범위 차이(예: 15 이상)를 강제해 균형 잡힌 분포를 유지합니다.

-  **포함/제외 번호 조건**: 사용자가 직접 선택한 포함 번호와 제외 번호를 반영  
  ↳ 특정 번호를 반드시 포함하거나 제외할 수 있어 개인화된 전략을 적용할 수 있습니다.

-  **합계 기준(내부 적용)**: 너무 낮거나 높은 번호 합계를 피함  
  ↳ 실제 당첨 번호 합계는 대체로 100~170 사이에 분포합니다. 극단적인 합계를 배제해 현실성 있는 조합을 유지합니다.
""")
